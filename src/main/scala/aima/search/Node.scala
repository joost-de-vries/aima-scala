...
sealed trait FailureLike[A] extends SearchResult[A]
case class Success[A](result: A) extends SearchResult[A]
case class Failure[A]() extends SearchResult[A] with FailureLike[A]
case class CutOff[A]() extends SearchResult[A]
  case class RBFSFailure[A](fVal: Double) extends FailureLike[A]()
