...
class Matrix[T: ClassTag] private (private val elems: Array[Array[T]])(n: Numeric[T]) {
...
object Matrix {
  ...
  def apply[T](elems: Array[Array[T]])(implicit n: Numeric[T],m: ClassTag[T]): Matrix[T] =
    new Matrix[T](clone(elems))(n)

  private def clone[T: ClassTag](arr: Array[Array[T]]): Array[Array[T]] = arr.map(_.clone)
}
